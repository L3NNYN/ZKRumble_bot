...
# --- Reglas de cada variante ---
defeats = {}

# Reglas para Fire, Well
rules_fw = {
    "Rock":     ["Scissors", "Fire"],
    "Paper":    ["Rock", "Well"],
    "Scissors": ["Paper", "Fire"],
...
async def handle_vote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if actuales >= MIN_PLAYERS and actuales == total:
        ...
        if selected_variation == "Rock, Paper, Scissors, Fire, Well":
            ...
            await update.message.reply_text("ℹ️ Las reglas de esta variante aún no han sido implementadas.")
        if selected_variation == "Rock, Paper, Scissors, Fire, Well":
            defeats.clear()
    ...
    for a1, a2 in current_matches:
        if anon_id in (a1, a2) and a1 in reveals and a2 in reveals:
            ...
            if j2 in defeats[j1]:
                ...
                for uid, pdata in players.items():
                    if pdata["anon_id"] in (a1, a2):
                        await context.bot.send_message(chat_id=uid, text="🤝 Empate. Ambos deben volver a jugar usando /play <jugada>.")
                reveals.pop(a1)
                reveals.pop(a2)
                return
            ...
            for uid in [uid1, uid2]:
                await context.bot.send_message(
                    chat_id=uid,
                    text=f"🎮 Resultado: {get_alias_from_anon(a1)} vs {get_alias_from_anon(a2)} 🏆 Ganador: {get_alias_from_anon(winner)}"
                )

            loser = a2 if winner == a1 else a1
            for uid, pdata in players.items():
                if pdata["anon_id"] == loser:
                    await context.bot.send_message(chat_id=uid, text="😢 Has sido eliminado del torneo. ¡Gracias por participar!")
            break
