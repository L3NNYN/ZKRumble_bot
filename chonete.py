...
async def reveal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for a1, a2 in current_matches:
        if anon_id in (a1, a2) and a1 in reveals and a2 in reveals:
            ...
            if j2 in defeats[j1]:
                winner = a1
                vc = hashlib.sha256(f"1{secrets.token_hex(4)}".encode()).hexdigest()
            elif j1 in defeats[j2]:
                winner = a2
                vc = hashlib.sha256(f"0{secrets.token_hex(4)}".encode()).hexdigest()
            else:
            ...
            victory_commits.setdefault(winner, []).append(vc)

            for uid in [uid1, uid2]:
                await context.bot.send_message(chat_id=uid, text=f"🎮 Resultado:\n{result}🏆 Ganador: {winner[:6]}")
            break
